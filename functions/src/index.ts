...
async function generateVideoFromTheme(theme : string) {
  ...
  const contentObject = JSON.parse(responseObject.content);
...
async function createVideo() {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    const cmd = `ffmpeg -loop 1 -i ${image} -i ${audioFile} -t ${duration} -filter_complex [0]scale=816:-2,setsar=1:1[out];[out]crop=816:1456[out];[out]scale=8000:-1,zoompan=z='zoom+0.001':x=${zoomPositionX}:y=${zoomPositionY}:d=${60*duration}:s=816x1456:fps=60[out] -map [out] -map 1:a -c:v libx264 -c:a aac -b:a 192k -pix_fmt yuv420p -y ${tempSegmentPath}`;
    await execAsync(cmd);
